...
const pkg = createRequire(import.meta.url)(join(__dirname, "../package.json")) as { version: string };
...
function dim(msg: string)  { log(`${c.gray}  ${msg}${c.reset}`); }
function step(msg: string) { log(`  ${arr} ${msg}`); }
function pass(msg: string) { log(`  ${ok} ${msg}`); }
function fail(msg: string) { log(`  ${err} ${c.red}${msg}${c.reset}`); }
function warn_(msg: string){ log(`  ${warn} ${c.yellow}${msg}${c.reset}`); }
...
async function waitForHealth(url: string, retries = 30, delayMs = 2000): Promise<boolean> {
...
function composeTemplate(apiKey: string): string {
  return `services:
  api:
...
function envTemplate(apiKey: string): string {
...
async function cmdInit(demo: boolean) {
  banner();
  ...
  if (demo) upArgs.push("--profile", "demo");
...
switch (subcommand) {
  case "init":   await cmdInit(demo); break;
  ...
  default:       await cmdInit(demo); break; // bare `npx operatorboard` = init
}
